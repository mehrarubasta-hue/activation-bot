...
async def add_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
  if update.effective_user.id != ADMIN_ID:
    return
  if not context.args:
    await update.message.reply_text("USAGE: /adduser <user_id> EXAMPLE: /adduser 871721883")
    return
...
async def add_word(update: Update, context: ContextTypes.DEFAULT_TYPE):
  if update.effective_user.id != ADMIN_ID:
    return
  if not context.args:
    await update.message.reply_text("USAGE: /addword word1, word2, word3 EXAMPLE: /addword spam, whatsapp, user id")
    return
